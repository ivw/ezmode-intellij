...
class TextObjectUtilsTest {
  ...
  fun findOpeningOrClosingDelim() {
    ...
    every { editor.document.charsSequence } returns chars
    PairDelim.parentheses.apply {
      findOpeningDelim(editor, 4, true)
        .shouldBe(2)
      findOpeningDelim(editor, 13, true)
        .shouldBe(2)
      findClosingDelim(editor, 12, true)
        .shouldBe(13)
      findClosingDelim(editor, 3, true)
        .shouldBe(13)
    }
  }
  ...
  fun selectTextObjectInsideDelimPair() {
    ...
    every { caret.editor.project } returns null
    ...
    verify { selectRange(DelimRanges(TextRange(1, 6)), caret, around = false, deleteDelims = false) }
  }
  ...
  fun selectTextObjectInsideEmptyDelimPair() {
    ...
    selectTextObject(caret, around = false, deleteDelims = false)
    verify { selectRange(DelimRanges(TextRange(1, 1)), caret, around = false, deleteDelims = false) }
  }
  ...
  fun selectTextObjectAroundEmptyDelimPair() {
    ...
    verify { selectRange(DelimRanges(TextRange(1, 1)), caret, around = true, deleteDelims = false) }
  }
